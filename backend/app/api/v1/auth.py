...
async def google_callback(
    ...
    Google OAuth callback endpoint.
    Google redirects here after user authentication.
    Exchanges code for user info and redirects back to Next.js with a JWT token query parameter.
    """
    # Step 1: Exchange code for user info
    user_info = await oauth_service.exchange_google_code(code)
    ...
    # ✅ FIX: Redirect the user back to the Vercel frontend domain with the secure access token appended
    frontend_url = settings.FRONTEND_URL.rstrip('/') if hasattr(settings, "FRONTEND_URL") else "http://localhost:3000"
    return RedirectResponse(url=f"{frontend_url}/auth/signin?token={token}")
